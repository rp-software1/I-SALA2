BLOQUE A:
• ¿La diferencia entre NEXT_PUBLIC_ y sin prefijo fue lo que predijiste?

Sí, la diferencia fue lo que había predicho. Aprendí que las variables con el prefijo NEXT_PUBLIC_ pueden ser utilizadas tanto en el navegador como en el servidor, mientras que las variables sin prefijo solo están disponibles en el servidor de Next.js. Esto es importante para proteger información sensible y también para configurar correctamente la conexión con APIs.

BLOQUE B:
¿El fetch en un Server Component sin useEffect fue más o menos complicado de lo que esperabas?
en realidad, no fue lo que esperaba, fue mucho mas sencillo, no necesite usar hooks, ni async await, fue muy intuitivo de usar.

BLOQUE C:
¿Fue intuitivo que loading.tsx y error.tsx son archivos especiales que Next.js detecta por nombre?
Sí, fue muy intuitivo. Next.js detecta automáticamente los archivos loading.tsx y error.tsx por nombre, lo que facilita mucho el manejo de estados de carga y errores.

BLOQUE D: 
¿Qué haría distinto en PlatoCard cuando llegue el Context en Día 3? ¿Qué línea va a cambiar?
PlatoCard dejará de usar solo estado local y llamará al PedidoContext para agregar platos al carrito global.

La línea que cambiará será handleAgregar:

agregarPlato(plato);

en lugar de solo mostrar el feedback visual temporal.
