...
// El Provider — envuelve los hijos que necesiten acceso al Context
export default function PedidoProvider({
    ...
    function agregarPlato(plato: Plato): void {
        setPedido(prev => {
            const existente = prev.items.find(i => i.platoId === plato.id);
            if (existente) {
                return {
                    ...prev,
            ...
            return {
                ...
                items: [
                    ...prev.items,
                    { platoId: plato.id, nombre: plato.nombre, cantidad: 1, precioUnitario: plato.precio },
                ],
                total: prev.total + plato.precio,
            };
        });
    ...
    function quitarPlato(platoId: string): void {
        setPedido(prev => {
            ...
            if (!item) return prev;
            if (item.cantidad === 1) {
                return { ...prev, items: prev.items.filter(i => i.platoId !== platoId), total: prev.total - item.precioUnitario };
            }
            return {
                ...prev,
                items: prev.items.map(i => i.platoId === platoId ? { ...i, cantidad: i.cantidad - 1 } : i),
